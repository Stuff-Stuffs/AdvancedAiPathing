...
import io.github.stuff_stuffs.advanced_ai_pathing.common.internal.pathing.NodeVisitedMap;
import io.github.stuff_stuffs.advanced_ai_pathing.common.internal.pathing.PathingHeap;
...
public abstract class AStar<N, C, T> {
    public static final int MAX_SUCCESSORS = 96;
    ...
    public PathInfo<N> findPath(final N start, final C context, final T target, final double error, final boolean partial) {
        ...
        while (!queue.isEmpty()) {
            ...
            if(current.heuristicDistance < bestDist && validEnd(current.delegate, context)) {
            ...
            if (shouldCutoff(current.delegate, context)) {
                continue;
            }
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (index < 0) {
                    ...
                    if (v + 0.01 < nodeCost(node.delegate)) {
    ...
    protected abstract boolean validEnd(N node, C context);

    protected abstract double heuristic(N node, T target, C context);

    protected abstract double nodeCost(N node);

    protected abstract boolean shouldCutoff(N node, C context);
